...
def defaults():
    ...
    env.service_name = ["gunicorn-arhitektuurkaks"]
    env.code_dir = '/srv/arhitektuurkaks'

@task
def test():
    defaults()
    env.hosts = ['maximum.thorgate.eu']
...
def deploy(id=None, silent=False, force=False):
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuurkaks')
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE arhitektuurkaks; '
         '      CREATE USER arhitektuurkaks WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE arhitektuurkaks to arhitektuurkaks;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/arhitektuurkaks/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/arhitektuurkaks/')
    with cd(env.code_dir + '/arhitektuurkaks'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/arhitektuurkaks/')

    # Copy logrotate, nginx, crontab and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/arhitektuurkaks')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/arhitektuurkaks' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-arhitektuurkaks.conf')
        sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuurkaks')
...
def collectstatic():
    with cd(env.code_dir + '/arhitektuurkaks'):
